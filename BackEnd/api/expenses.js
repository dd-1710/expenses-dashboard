...
const VALID_CATEGORIES = ['Food','Shopping','Vacation','Health','Insurance','Transportation','Entertainment','Bills','Education','Investment','Others'];
...
router.get('/get-all-expenses', authMiddleware, async (req, res) => {
    try {
        ...
        // Current-month boundaries for budget-relevant queries (UTC)
        const now = new Date();
        const monthStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1));
        const monthEnd = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() + 1, 1));
        const monthMatch = { userId: userObjId, date: { $gte: monthStart, $lt: monthEnd } };

        const foundExpense = await expense.find({ userId }).sort({ date: -1 }).skip(skip).limit(limit);
        ...
        const monthlyData = await expense.aggregate([
            { $match: { userId: userObjId, date: { $gte: new Date('2026-01-01T00:00:00.000Z') } } },
        ...
        return res.status(500).json({ message: 'Unable to fetch the expenses' })
    }
...
router.delete('/delete-expense/:id',authMiddleware,async(req,res)=>{
  try{
    ...
    return res.status(500).json({message:'Unable to delete the expense'})
  }
})
...
router.put('/update-expense/:id',authMiddleware,async(req,res)=>{
    try{
        ...
        if(typeof amount !== 'number' || amount <= 0 || amount > 10000000){
            return res.status(400).json({message:'Amount must be between 1 and 1,00,00,000'});
        }
        ...
        const updateExpense = await expense.findByIdAndUpdate(id,{amount,category,description: typeof description === 'string' ? description.trim().slice(0, 120) : '',date},{new:true})
